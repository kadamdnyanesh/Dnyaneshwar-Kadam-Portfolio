<script lang="ts">
  import { portfolioData, getPersonalSummary } from "../data/portfolio";
  import { reveal } from "./actions/reveal";

  const currentYear = new Date().getFullYear();
  const { name, socialLinks } = portfolioData.personalDetails;
  const summary = getPersonalSummary();
</script>

<footer class="bg-white dark:bg-gray-800 py-8 mt-12" use:reveal>
  <div class="container mx-auto px-4 max-w-4xl">
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
      ...
      <div>
        ...
        <div class="flex space-x-4">
          ...
          <a
            href={socialLinks.codeSandbox}
            target="_blank"
            rel="noopener noreferrer me"
            aria-label="CodeSandbox profile of {name}"
            class="text-slate-600 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400"
          >
            <svg
    ...
    >
      <p>
        <small>&copy; {currentYear} {name}. All rights reserved.</small>
      </p>
    </div>
  </div>
</footer>
